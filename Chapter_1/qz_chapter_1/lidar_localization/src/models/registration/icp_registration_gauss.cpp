 ...
 * @Author: QZ
 * @Date: 2022-06-30 10:30:27
 */
#include "lidar_localization/models/registration/icp_registration_gauss.hpp"
#include <Eigen/Core>
#include "glog/logging.h"

namespace lidar_localization
{
ICPGaussRegistration::ICPGaussRegistration(const YAML::Node& node)
...
ICPGaussRegistration::ICPGaussRegistration(float max_dist, int max_iter)
  : kdtree_ptr_(new pcl::KdTreeFLANN<CloudData::POINT>)
{
  SetRegistrationParam(max_dist, max_iter);
}

bool ICPGaussRegistration::SetRegistrationParam(float max_dist, int max_iter)
{
  max_disttance_ = max_dist;
...
bool ICPGaussRegistration::SetInputTarget(const CloudData::CLOUD_PTR& input_target)
{
...
bool ICPGaussRegistration::ScanMatch(const CloudData::CLOUD_PTR& input_source, const Eigen::Matrix4f& predict_pose,
...
void ICPGaussRegistration::calculateTrans(const CloudData::CLOUD_PTR& input_source)
{
  ...
  while (iterator_num < max_iterator_)
  {
    ...
    for (size_t i = 0; i < transformed_cloud->size(); ++i)
    {
      ...
      if (distasnces[0] > max_disttance_)
      {
        continue;
      }
