...
bool FrontEnd::InitRegistration(std::shared_ptr<RegistrationInterface>& registration_ptr, const YAML::Node& config_node) {
    ...
    else if (registration_method == "ICPGAUSS")
    {
        LOG(INFO) << " 报错! ";
        registration_ptr = std::make_shared<ICPGaussRegistration>(config_node[registration_method]);
    }
    else 
    {
...
bool FrontEnd::Update(const CloudData& cloud_data, Eigen::Matrix4f& cloud_pose) {
    ...
    // 更新相邻两帧的相对运动 用运动模型预测最新帧的位姿，此处的current_frame_代表的上一帧，predict_pose代表的是当前帧。
