...
void SCManager::makeAndSaveScancontextAndKeys(pcl::PointCloud<SCPointType>& _scan_down)
{
  ...
  if (scan_context_img_.cols == 0)
  {
    scan_context_img_.create(PC_NUM_RING, PC_NUM_SECTOR, CV_8UC1);
  }

  MatrixXd::Index maxRow, maxCol;
  MatrixXd::Index minRow, minCol;
  float max_data = sc.maxCoeff(&maxRow, &maxCol);
  float min_data = sc.minCoeff(&minRow, &minCol);
  float scale = 255 / (max_data - min_data);
  for (auto i = 0; i < scan_context_img_.rows; i++)
  {
    for (auto j = 0; j < scan_context_img_.cols; j++)
    {
      scan_context_img_.at<uint8_t>(i, j) = (uint8_t)(sc(i, j) - min_data) * scale;
    }
  }
  // std::cout <<polarcontext_vkeys_.size() << std::endl;
