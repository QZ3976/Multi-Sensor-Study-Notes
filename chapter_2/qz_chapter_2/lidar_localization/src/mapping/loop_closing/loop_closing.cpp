...
bool LoopClosing::InitParam(const YAML::Node& config_node)
{
  ...
  loop_method_ = config_node["loop_method"].as<std::string>();
  std::cout << "闭环检测关键帧的方法为： " << loop_method_ << std::endl << std::endl;
...
bool LoopClosing::InitDataPath(const YAML::Node& config_node)
{
  ...
  std::string loop_file = data_path + "/slam_data/loop_pose_" + loop_method_ + ".txt ";
  loop_out_file_.open(loop_file.c_str());
...
bool LoopClosing::Update(const KeyFrame key_frame, const KeyFrame key_gnss)
{
  ...
  // 添加sc回环检测功能
  else if (loop_method_ == "sc")
  {
    auto idx = key_frame.index;
    std::string key_frame_filename = key_frames_path_ + "/key_frame_" + std::to_string(idx) + ".pcd";
    CloudData::CLOUD_PTR key_frame_cloud_ptr(new CloudData::CLOUD());
    pcl::io::loadPCDFile(key_frame_filename, *key_frame_cloud_ptr);
    sc_manager_->makeAndSaveScancontextAndKeys(*key_frame_cloud_ptr);
    std::string key_frame_scan_context = key_frames_path_ + "/key_frame_" + std::to_string(idx) + "_context.png";
...
bool LoopClosing::DetectNearestKeyFrameScanContext(int& key_frame_index)
{
  static int skip_cnt = 0;
  static int skip_num = loop_step_;
  if (++skip_cnt < skip_num)
  ...
  KeyFrame current_key_frame = all_key_gnss_.back();
  double dx = current_key_frame.pose(0, 3) - all_key_frames_[detectResult.first].pose(0, 3);
  double dy = current_key_frame.pose(1, 3) - all_key_frames_[detectResult.first].pose(1, 3);
  double dz = current_key_frame.pose(2, 3) - all_key_frames_[detectResult.first].pose(2, 3);
  double distance = sqrt(dx * dx + dy * dy + dz * dz);
  skip_cnt = 0;
  skip_num = (int)distance;
  if (distance > detect_area_)
  {
    skip_num = std::max((int)(detect_area_ / 2.0), loop_step_);
    return false;
  }
  else
  {
...
bool LoopClosing::CloudRegistration(int key_frame_index)
{
  ...
  std::cout << "检测到闭环 " << loop_close_cnt << ": 帧" << current_loop_pose_.index0 << "------>"
            ...
            << std::endl;
  loop_out_file_ << "检测到闭环 " << loop_close_cnt << ": 帧" << current_loop_pose_.index0 << "------>"
                 << "帧" << current_loop_pose_.index1 << std::endl
                 << "fitness score: " << registration_ptr_->GetFitnessScore() << std::endl
                 << std::endl;
...
bool LoopClosing::Registration(CloudData::CLOUD_PTR& map_cloud_ptr, CloudData::CLOUD_PTR& scan_cloud_ptr,
